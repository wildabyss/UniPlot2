...
var Plotter = function () {
	...
	var setLineStyle = function setLineStyle(ctx, file_counter, param_counter) {
		if (file_counter == 0) ctx.setLineDash([]);else ctx.setLineDash([2 / file_counter, 3 / file_counter]);
  ...
  */
	var plot = function plot(data_sources, canvas_container, plot_parameters) {
		// parse plot_parameters
		if (!plot_parameters instanceof PlotParameters) throw "second parameter must be an instance of plot_graph_data class";
		var minX = plot_parameters.hor_zoom[0],
		    maxX = plot_parameters.hor_zoom[1];
		var minY_pri = plot_parameters.pri_vert_zoom[0],
		    maxY_pri = plot_parameters.pri_vert_zoom[1];
		var minY_sec = plot_parameters.sec_vert_zoom[0],
		    maxY_sec = plot_parameters.sec_vert_zoom[1];
		...
		var projectCoordinates = function projectCoordinates(zoom_bounds_x, zoom_bounds_y, x, y) {
		...
		for (var parameter_name in plot_parameters.parameters) {
			...
			for (var source_name in data_sources) {
				...
				for (var i = 0; i < data_source.data[parameter_name].length - 1; i++) {
					var coords_start = projectCoordinates(plot_parameters.hor_zoom, parameter.is_primary ? plot_parameters.pri_vert_zoom : plot_parameters.sec_vert_zoom, data_source.data[plot_parameters.xvar][i], data_source.data[parameter_name][i]);
					var coords_end = projectCoordinates(plot_parameters.hor_zoom, parameter.is_primary ? plot_parameters.pri_vert_zoom : plot_parameters.sec_vert_zoom, data_source.data[plot_parameters.xvar][i + 1], data_source.data[parameter_name][i + 1]);
				...
				if (!active_once) num_active_sources++;
				file_counter++;
			}

			// counters for primary and secondary axes
			if (parameter.is_primary) N_primary++;else N_secondary++;
		...
		for (var parameter_name in plot_parameters.parameters) {
			...
			for (var source_name in data_sources) {
				...
				ctx.textBaseline = "top";
				ctx.font = label_font + "px Arial";
				ctx.fillText(parameter_name + (plot_parameters.parameters[parameter_name].unit == "" ? "" : " (" + plot_parameters.parameters[parameter_name].unit + ")") + (Object.keys(data_sources).length <= 1 ? "" : " [" + source_name + "]"), legend_x, legend_y);
				ctx.stroke();

				file_counter++;
			}

			if (plot_parameters.parameters[parameter_name].is_primary) N_primary++;else N_secondary++;
		}
	};

	var parseDataLine = function parseDataLine(data_sources, file_name, line, line_index) {
		var delimiter = '';
		var title_line = 0;
		...
		if (line_index < title_line) return true;else {
			...
			split_arr.every(function (el) {
				...
				if (terminator !== false && el.startsWith(terminator)) {
					...
					if (el == 't') el = 'time';else if (el == "") return true; // skip iteration

					if (!data_sources[file_name].data.hasOwnProperty(el)) data_sources[file_name].fields.push(el);
					data_sources[file_name].data[el] = [];
				} else {
					// data line

					if (el == "") return true; // skip iteration
					else {
			...
			if (terminator_reached) return false;
		}
	...
	return {
		...
		redraw: function redraw() {
			...
			// sanity check
			if (this.plot_parameters_array.length != containers.length) throw "data lengths mismatch";
		...
		zoomHorizontal: function zoomHorizontal(canvas_index, zoom_coordinates, no_redraw) {
			var getBounds = function getBounds(x, canvas, plot_parameters) {
				// total canvas dimensions
			...
			if (no_redraw === undefined) no_redraw = false;
			if (!no_redraw) this.redraw();
		},

		zoomVertical: function zoomVertical(canvas_index, zoom_coordinates, no_redraw) {
			var getBounds = function getBounds(y, canvas, plot_parameters, is_primary) {
				// total canvas dimensions
				...
				if (is_primary) return (y - top_offset) / vert_res_with_offset * (plot_parameters.pri_vert_zoom[0] - plot_parameters.pri_vert_zoom[1]) + plot_parameters.pri_vert_zoom[1];else return (y - top_offset) / vert_res_with_offset * (plot_parameters.sec_vert_zoom[0] - plot_parameters.sec_vert_zoom[1]) + plot_parameters.sec_vert_zoom[1];
			};
