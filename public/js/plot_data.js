...
function DataSource(active) {
	if (typeof active != "boolean") throw "parameter must be a boolean";
...
PlotParameters.prototype.Parameter = function (unit, is_primary) {
	if (typeof unit != "string") throw "second parameter must be a string";
	if (typeof is_primary != "boolean") throw "third parameter must be a boolean";
...
PlotParameters.prototype.addParameter = function (data_sources, parameter_name, unit, is_primary) {
	...
	if (!this.parameters.hasOwnProperty(parameter_name)) {
		if (is_primary) this.size_primary++;else this.size_secondary++;
	}
...
PlotParameters.prototype.removeParameter = function (data_sources, parameter_name) {
	if (!this.parameters.hasOwnProperty(parameter_name)) return;
...
PlotParameters.prototype.setZoomOnParameter = function (data_sources, parameter_name) {
	for (source_name in data_sources) {
		...
		if (parameter_name == this.xvar) {
			// horizontal zoom
			if (this.hor_zoom[0] == this.hor_zoom[1]) this.setHorizontalZoom(data_source.data[parameter_name].min(), data_source.data[parameter_name].max());else this.setHorizontalZoom(Math.min(this.hor_zoom[0], data_source.data[parameter_name].min()), Math.max(this.hor_zoom[1], data_source.data[parameter_name].max()));
		} else if (this.parameters[parameter_name].is_primary) {
			// primary vertical zoom
			if (this.pri_vert_zoom[0] == this.pri_vert_zoom[1]) this.setPriVerticalZoom(data_source.data[parameter_name].min(), data_source.data[parameter_name].max());else this.setPriVerticalZoom(Math.min(this.pri_vert_zoom[0], data_source.data[parameter_name].min()), Math.max(this.pri_vert_zoom[1], data_source.data[parameter_name].max()));
		} else {
			// secondary vertical zoom
			if (this.sec_vert_zoom[0] == this.sec_vert_zoom[1]) this.setSecVerticalZoom(data_source.data[parameter_name].min(), data_source.data[parameter_name].max());else this.setSecVerticalZoom(Math.min(this.sec_vert_zoom[0], data_source.data[parameter_name].min()), Math.max(this.sec_vert_zoom[1], data_source.data[parameter_name].max()));
		}
	}
};
...
PlotParameters.prototype.revertZoom = function (data_sources, horizontal_only) {
	if (horizontal_only == undefined || horizontal_only) this.setHorizontalZoom(0, 0);
